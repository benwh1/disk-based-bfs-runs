...
pub fn decode_depth_13_positions() {
    let depth_13s = [
        2116315323,
        2522922380,
        2522922385,
    ...
    for (i, &pos) in depth_13s.iter().enumerate() {
        let mut cube = Cube::new();
        cube.decode(pos);

        let mut s = format!("MaxSolutions 1\nScramble s{i}\ncorners\n");
        for i in 0..8 {
            s.push_str(&format!("{} ", cube.cp[i] + 1));
        }
        s.pop();
        s.push('\n');
        for i in 0..8 {
            s.push_str(&format!("{} ", cube.co[i]));
        }
        s.pop();
        s.push('\n');
        s.push_str("edges\n");
        for i in 0..12 {
            s.push_str(&format!("{} ", cube.ep[i] + 1));
        }
        s.pop();
        s.push('\n');
